...
visualize_model <- function(modobj,
  ...
  if (!is.null(int_var)) {
    ...
    p<-data.frame(predict(model,pred,se.fit = T,type="response",unconditional=TRUE))
    pred <- cbind(pred,p)
